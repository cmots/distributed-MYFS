             ...
             struct fuse_file_info *fi)
{
    ...
    if (global_offset != GLOBAL_BUFF_SIZE)
    {
        memcpy(global_buff + global_offset, buf, size);
        global_offset += size;
        return 0;
    }
    else
    {
        return log_syscall("myfs_write", myfs_write(path);)
    }
    //Although it's a good idea to do remote operation after local writing
...
int bb_release(const char *path, struct fuse_file_info *fi)
{
    ...
    if (is_write)
    {
        is_write = 0;
        log_syscall("close", close(fi->fh), 0);
        return log_syscall("myfs_flush", myfs_flush(path));
